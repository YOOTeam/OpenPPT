  ...
  <div class="color-picker">
    ...
    <div class="picker-presets">
      <div
        ...
        v-for="(c, index) in themeColors"
        :key="c + index"
        :class="[modelValue === initColor(c) ? 'activeColor' : '']"
        :style="{ background: initColor(c) }"
        @click="selectThemePresetColor(c)"
      ></div>
    </div>
    ...
    <div class="picker-presets">
      <div
        v-for="(c, index) in standardColors"
        :key="c + index"
        class="picker-presets-color"
        :style="{ background: c }"
        :class="[modelValue === c ? 'activeColor' : '']"
...
const gradient = (
  ...
  const currentColorData = themeColors[index]
  const color = initColor(currentColorData)
  const isLight = tinycolor(color).isLight()
  let brightnessArry = brightnessDarkRang[index]
  const bigRang = [1, 2, 3, 4, 5, 8, 9]

  if (
    (!isLight && !bigRang.includes(index)) ||
    (isLight && bigRang.includes(index))
  ) {
    brightnessArry = brightnessLightRang[index]
  }
...
const getPresetColors = () => {
  const presetColors = []
  for (const [index, color] of presetColorConfig.entries()) {
    presetColors.push(gradient(color[1], color[0], 5, index))
  }
...
let presetColors = getPresetColors()
...
const selectThemePresetColor = (colorData: any) => {
  const colorString = initColor(colorData)
  hue.value = tinycolor(colorString).toHsl().h
  emit('update:modelValue', colorData)
}
...
const browserEyeDropper = () => {
  message.success(t('canvas.browserEyeDropper'), { duration: 0 })
...
const customEyeDropper = () => {
  ...
  colorBlockRef.style.cssText =
    'position: absolute; top: -100px; left: -100px; width: 50px; height: 50px; border: 1px solid #fff;border-radius:50%; z-index: 999'
  ...
  toCanvas(targetRef, {
    ...
    .catch(() => {
      message.error(t('canvas.browserError'))
      document.body.removeChild(maskRef)
    })
}
</script>
...
.picker-color-wrap {
  ...
  margin-top: 4rem;
  margin-right: 4rem;
  outline: 1rem dashed rgba($color: #666, $alpha: 0.12);
