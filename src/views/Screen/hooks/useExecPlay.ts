...
import useAnimmation from './useAnimmation'
import $ from 'jquery'
...
export default () => {
  ...
  const { slides, slideIndex, formatedAnimations, currentSlide } =
  ...
  const setLoopPlay = (loop: boolean) => {
    ...
    if (loop && autoPlayTimer.value === 0) {
      setAutoPlayInterval(2500)
    }
  }
  ...
  const execPrev = (e?: any) => {
    ...
    if (formatedAnimations.value.length && animationIndex.value > 0) {
      ...
      else throttleMassage(t('baseView.homePage'))
    }
    inAnimation.value = false
  }
  const execNext = (e?: any) => {
    isEndPage.value = false
    if (
      !(e instanceof WheelEvent) &&
      actionAnimation.value &&
    ...
    if (
      ...
      if (loopPlay.value) turnSlideToIndex(0)
        ...
        throttleMassage(
          autoPlayTimer.value ? t('baseView.overPlay') : t('play.endMessage')
        )
        closeAutoPlay()
      }
      inAnimation.value = false
  ...
  const autoPlay = () => {
    closeAutoPlay()
    message.success(t('play.startAutoPlay'))
  ...
  const mousewheelListener = throttle(
    function (e: WheelEvent) {
      if (e.deltaY < 0) execPrev(e)
      else if (e.deltaY > 0) execNext(e)
    },
    500,
    { leading: true, trailing: false }
  )
  ...
  const keydownListener = (e: KeyboardEvent) => {
    ...
    if (key === KEYS.UP || key === KEYS.LEFT || key === KEYS.PAGEUP) execPrev(e)
    else if (
      key === KEYS.DOWN ||
      key === KEYS.RIGHT ||
      key === KEYS.SPACE ||
