...
const themeColors = computed(() => {
  ...
  if (props.themeColors.length >= 10) colors = props.themeColors
    ...
    colors = tinycolor(initColor(props.themeColors[0]))
      .analogous(10)
      .map((color) => {
    ...
    const supplement = tinycolor(initColor(props.themeColors[len - 1]))
      .analogous(10 + 1 - len)
      .map((color) => color.toRgbString())
    const upkey = props.themeColors.slice(0, len - 1).map((color) => {
      return initColor(color)
    })
    colors = [...upkey, ...supplement]
  }
  return colors
})
...
const updateOption = () => {
  const option = getChartOption({
    ...
    textColor: initColor(props.textColor),
    lineSmooth: props.options?.lineSmooth || false,
    stack: props.options?.stack || false,
    legend: props.legend || 'bottom',
    labelValue: props.labelValue || 'none',
