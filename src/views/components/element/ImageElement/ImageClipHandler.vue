  ...
  >
    ...
    <div
      ...
      :style="{
        ...
        clipPath: clipData?.path ? `url(#clip-shape-${id}})` : clipPath,
      }"
    >
      <img
        class="top-img"
    ...
    <svg height="0" width="0" v-if="clipData?.path">
      <defs>
        <clipPath :id="`clip-shape-${id}}`">
          <path
            :d="clipData.path"
            :transform="`scale(${clipData.viewBox[0] / resizeWidth}, ${
              clipData.viewBox[1] / resizeHeight
            })`"
          ></path>
        </clipPath>
      </defs>
...
let resizeWidth = props.width
let resizeHeight = props.height
...
const scaleClipRange = (e: MouseEvent, type: OperateResizeHandlers) => {
  ...
  document.onmousemove = (e) => {
    ...
    const box = document.getElementsByClassName('top-image-content')[0]
    resizeWidth = box.offsetWidth
    resizeHeight = box.offsetHeight
  }
