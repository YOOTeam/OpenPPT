<template>
  <div class="editable-table" :style="{ width: totalWidth + 'px' }">
    ...
    <table
      ...
      :style="`--themeColor: ${initColor(theme?.color)}; --subThemeColor1: ${
        subThemeColor[0]
      ...
      <tbody>
        ...
        >
          <td
            ...
            :style="{
              ...TableStyle(rowIndex, colIndex, rowCells),
...
const TableStyle = computed(
  (rowIndex, colIndex, rowCells) => (rowIndex, colIndex, rowCells) => {
    let table = {
      borderStyle: props.outline.style,
      borderColor: initColor(props.outline.color),
      borderWidth: props.outline.width + 'px',
    }

    if (props?.tableStyle === 'threeTable') {
      table = {
        borderLeftWidth: 0,
        borderStyle: props.outline.style,
        borderColor: initColor(props.outline.color),
      ...
      if (rowIndex === 0 || rowIndex === props.data.length - 1) {
        table.borderBottomWidth = parseInt(props.outline.width + 3) + 'px'
        table.borderLeftWidth = props.outline.width + 'px'
        if (rowIndex === 0) {
          table.borderTopWidth = parseInt(props.outline.width + 3) + 'px'
        }
      } else {
        table = {
          borderStyle: props.outline.style,
      ...
      if (colIndex === 0) {
        table.borderLeftWidth = 0
      }
      if (colIndex === rowCells.length - 1) {
        table.borderRightWidth = 0
      }
    }
...
const contextmenus = (el: HTMLElement): ContextmenuItem[] => {
  ...
  return [
    {
      text: t('lastReamrk.insertCol'),
      children: [
        { text: t('lastReamrk.inserLeft'), handler: () => insertCol(colIndex) },
        {
          text: t('lastReamrk.inserRight'),
          handler: () => insertCol(colIndex + 1),
        },
      ],
    },
    {
      text: t('lastReamrk.insertRow'),
      children: [
        {
          text: t('lastReamrk.deleteColUp'),
          handler: () => insertRow(rowIndex),
        },
        {
          text: t('lastReamrk.deleteColDown'),
          handler: () => insertRow(rowIndex + 1),
        },
      ],
    },
    {
      text: t('lastReamrk.deleteCol'),
      disable: !canDeleteCol,
      handler: () => deleteCol(colIndex),
    },
    {
      text: t('lastReamrk.deleteRow'),
      disable: !canDeleteRow,
      handler: () => deleteRow(rowIndex),
    },
    { divider: true },
    {
      text: t('lastReamrk.canMerge'),
      disable: !canMerge,
      handler: mergeCells,
    },
    {
      text: t('lastReamrk.cancleCancelMerge'),
      disable: !canSplit,
      handler: () => splitCells(rowIndex, colIndex),
    },
    { divider: true },
    {
      text: t('lastReamrk.selectCol'),
      handler: () => selectCol(colIndex),
    },
    {
      text: t('lastReamrk.selectRow'),
      handler: () => selectRow(rowIndex),
    },
    {
      text: t('lastReamrk.selectAll'),
      handler: selectAll,
    },
  ]
}
