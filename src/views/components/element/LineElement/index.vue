      ...
      <svg overflow="visible" :width="svgWidth" :height="svgHeight">
        <defs>
          ...
          <LinePointMarker
            ...
            :color="markerColor('end')"
            :baseSize="elementInfo.width"
          />
          <GradientDefs
            v-if="elementInfo.gradient"
            :id="`editabel-gradient-line-${elementInfo.id}`"
        ...
        <path
          ...
          :stroke="
            elementInfo.gradient
              ? `url(#editabel-gradient-line-${elementInfo.id})`
              : initColor(elementInfo.color)
          "
...
const props = defineProps<{
  ...
  dragElement: (e: MouseEvent | TouchEvent, element: PPTLineElement) => void
  contextmenus: () => ContextmenuItem[] | null
}>()

const markerColor = computed(() => (type: string) => {
  let color = ''
  if (props.elementInfo.gradient) {
    const gradientColor = props.elementInfo.gradient.colors
    color = gradientColor[0].color
    if (type === 'end') {
      color = gradientColor[gradientColor.length - 1].color
    }
  } else {
    color = props.elementInfo.color
  }
