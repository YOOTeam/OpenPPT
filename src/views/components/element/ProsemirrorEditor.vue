<template>
  <div
    class="prosemirror-editor prosemirror-editor-text"
...
const emit = defineEmits<{
  ...
  (event: 'removerPlaceHolder', payload: boolean): void
}>()
...
const innerHTML = ref('')
function removeTextFromHTML(html: string, text?: string): string {
  // 创建一个新的 DOM 解析器
  const parser = new DOMParser()
  // 使用解析器将 HTML 字符串转换为 DOM 对象
  ...
  while ((node = walker.nextNode())) {
    if (node.nodeType === 3) {
      node.parentNode.innerHTML = text ? text : ''
    }
...
const placeHolderAttr = ref(null)
const handleFocus = () => {
...
// 暴露 focus 方法
const focus = () => {
  // 先刷新dom，以应对 props.editable 属性以更改，但当前实例还未捕获
  nextTick(() => {
    editorView.focus()
  })
}
...
const execCommand = ({ target, action }: RichTextCommand) => {
  ...
  const colorObj = action?.colorObj || null
  ...
  for (const item of actions) {
    ...
    } else if (item.command === 'cursor2tail') {
      // 将光标移至编辑区尾部
      const { state, dispatch } = editorView
      const tr = state.tr
      const cursor = TextSelection.create(tr.doc, tr.doc.content.size)
...
onMounted(() => {
  ...
  editorViewRef.value.addEventListener(
    'focus',
    function () {
      if (!isPC()) {
        mainStore.setMobileEditor(false)
      }
      // 可以在这里添加获得焦点时的样式变化或其他逻辑
    },
    true
  )
})
...
<style>
/* 针对a标签样式 */
.ProseMirror-static.prosemirror-editor-text a,
.prosemirror-editor.prosemirror-editor-text a {
