<template>
  <div class="chart-data-editor">
    ...
    <div class="btns">
      <div class="left">
        <Button class="btn" @click="clear()">{{ $t('toolbar.clear') }}</Button>
        <Button
          type="primary"
          class="btn"
          @click="getTableData()"
          style="margin-left: 10px"
          >{{ $t('toolbar.sure') }}</Button
        >
      </div>
      <!-- <div class="right">
...
const getTableData = () => {
  ...
  for (let rowIndex = 1; rowIndex < row; rowIndex++) {
    let labelsItem = `${t('toolbar.class')}${rowIndex}`
  ...
  for (let colIndex = 1; colIndex < col; colIndex++) {
    let legendsItem = `${t('toolbar.series')}${colIndex}`
    const labelInputRef = document.querySelector(
      `#cell-0-${colIndex}`
    ) as HTMLInputElement
