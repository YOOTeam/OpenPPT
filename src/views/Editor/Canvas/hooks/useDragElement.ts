...
export default (
  ...
  const { activeElementIdList, activeGroupElementId, drageTextCursor } =
  ...
  const isElementCompletelyHidden = (element: any, container: any) => {
    ...
    // 4. 完全在容器下方
    return (
      elementRect.right < containerRect.left || // 完全在左边
      elementRect.left > containerRect.right || // 完全在右边
      elementRect.bottom < containerRect.top || // 完全在上边
      elementRect.top > containerRect.bottom // 完全在下边
    )
  }
  ...
  const dragElement = (e: MouseEvent | TouchEvent, element: PPTElement) => {
    ...
    const dom = document.getElementById(`eidtor-text-${element.id}`)
    const dragDom = document.getElementById(`editable-element-${element.id}`)

    const container = document.querySelector('.viewport')
    if (dom) {
      dom.style.cursor = 'move'
      mainStore.setDrageTextCursor('move')
    }
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      mainStore.setDrageElement(true)
      const currentPageX =
    ...
    const handleMouseup = (e: MouseEvent | TouchEvent) => {
      if (dom) {
        ...
        mainStore.setDrageTextCursor('')
      }

      mainStore.setDrageElement(false)
      isMouseDown = false
      ...
      if (dragDom) {
        // 较新的API，检查更全面但兼容性有限
        const isCompletelyHidden = isElementCompletelyHidden(
          dragDom.children[0],
          container
        )
        // 元素完全不可见  删除元素
