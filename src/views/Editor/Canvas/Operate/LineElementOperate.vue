...
const { canvasScale, showLoadingMarks } = storeToRefs(useMainStore())
...
const tooberTop = computed(() => {
  ...
  if (props.elementInfo.top < 0) {
    const bottom = resizeHandlers.value.find((item) => item.handler === 'end')
    if (bottom) {
      top = parseInt(bottom.style.top) + 8 + 'px'
    }
  } else {
    top = '-60px'
  }
