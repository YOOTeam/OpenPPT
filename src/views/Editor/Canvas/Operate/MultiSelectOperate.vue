...
const { activeElementIdList, canvasScale, showLoadingMarks } = storeToRefs(
  useMainStore()
)
...
const tooberLeft = computed(() => {
  const datas = borderLines.value.find((item) => item.type === 'top')
  let w = range.value.minX / 2
  if (datas?.style?.width) {
    w = parseInt(datas?.style?.width) / 2 + 30
  }
  return w + 'px'
})
