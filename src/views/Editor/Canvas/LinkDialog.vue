<template>
  <div class="link-dialog">
    ...
    <div class="btns">
      ...
      <Button type="primary" @click="save()">{{ $t('toolbar.sure') }}</Button>
    </div>
  </div>
</template>
...
const slideOptions = computed(() => {
  return slides.value.map((item, index) => ({
    label: `${t('catalog.titleTab2')} ${index + 1}`,
    value: item.id,
...
const tabs: TabItem[] = [
  { key: 'web', label: t('lastReamrk.urlLink') },
  { key: 'slide', label: t('lastReamrk.slidePage') },
]
