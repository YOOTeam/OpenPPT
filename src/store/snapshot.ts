  ...
  actions: {
    ...
    async initSnapshotDatabase() {
      ...
      const newFirstSnapshot = {
        ...
        themeColorList: slidesStore.themeColorList,
        resourcesData: slidesStore.resourcesData,
      }
      await db.snapshots.add(newFirstSnapshot)

      this.setSnapshotCursor(0)
      this.setSnapshotLength(1)
      this.actionTime = moment(Date.now()).format('YYYY-MM-DD HH:mm:ss')
    },
    ...
    async unDo() {
      ...
      const { index, slides, themeColorList, resourcesData } = snapshot

      const slideIndex = index > slides.length - 1 ? slides.length - 1 : index

      slidesStore.setSlides(slides)
      slidesStore.setResourcesData(resourcesData)
    ...
    async reDo() {
      ...
      slidesStore.setSlides(slides)
      slidesStore.setThemeColorList(themeColorList)
      slidesStore.setThemeColorList(resourcesData)
      slidesStore.updateSlideIndex(slideIndex)
      this.setSnapshotCursor(snapshotCursor)
      mainStore.setActiveElementIdList([])
    },
