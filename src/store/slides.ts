...
export const useSlidesStore = defineStore('slides', {
  state: (): SlidesState => {
    const { t } = useI18n()
    return {
      defualtTitle: t('canvas.slideTitle'),
      oldTilte: t('canvas.slideTitle'), // 旧的幻灯片标题
      title: t('canvas.slideTitle'), // 幻灯片标题
      themeColorList: {
        text1: '#1726c7',
        bg1: '#FFFFFF',
        text2: '#0714a8',
      ...
      theme: theme, // 主题样式
      saveTime: moment(Date.now()).format('YYYY-MM-DD HH:mm:ss'),
      slides: [], // 幻灯片页面数据 slides
      slideIndex: 0, // 当前页面索引
      viewportSize: 1000, // 可视区域宽度基数
      viewportRatio: 0.5625, // 可视区域比例，默认16:9
      catalogType: 'image', // 导航样式 大纲 image, title 默认images
      resourcesData: null,
      oldSlides: [],
      pptData: null,
      // oldDataImgList: [],
  ...
  actions: {
    updataPPTViewData(data: any) {
      this.pptViewData = { ...this.pptViewData, ...data }
    },
    setPPTView(data: any) {
      this.pptViewData = data
    },
    setOldTilte(data: any) {
      this.oldTilte = data
    },
    setMobildSlideClick(data: any) {
      this.mobildSlideClick = data
    },
    // setOldDataImgList(data: any) {
    //   this.oldDataImgList = data
    // },
    setPPTData(data: any) {
      this.pptData = data
    },
    updataPPTData(data: any) {
      this.pptData = { ...this.pptData, ...data }
    },
    setResourcesData(data: any) {
      this.resourcesData = data
    },

    setOldSlides(data: any) {
      this.oldSlides = data
    },

    updateResourcesData(data: any) {
      this.resourcesData = { ...this.resourcesData, ...data }
    },
    setThemeColorList(data: any) {
      this.themeColorList = data
    },
    setThemeColorItem(themeProps: any) {
      this.themeColorList = { ...this.themeColorList, ...themeProps }
    },
    setTitle(title: string) {
      if (!title) {
        this.title = this.defualtTitle
        this.oldTilte = this.defualtTitle
      } else {
        this.title = title
      }
    },
    ...
    addSlideAsIndex(index: number, slide: Slide | Slide[]) {
      const slides = Array.isArray(slide) ? slide : [slide]
      this.slides.splice(index, 0, ...slides)
